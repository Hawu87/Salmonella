...
export default function CooccurrenceNetwork({ onNodeClick }: CooccurrenceNetworkProps) {
  ...
  const graphData = useMemo(() => {
    if (!data?.cooccurrence) return { nodes: [], links: [] };
    ...
    return {
      nodes: nodes.map(node => ({
        id: node.id, name: node.id, value: node.count,
        size: Math.sqrt(node.count / maxNodeCount) * 15 + 5,
      })),
      links: links.map(link => ({
        source: link.source, target: link.target, value: link.count,
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      if (isMobile) setDimensions({ width: window.innerWidth - 32, height: 500 });
      else if (isTablet) setDimensions({ width: window.innerWidth - 64, height: 600 });
      else setDimensions({ width: 1000, height: 700 });
    };
    updateDimensions();
  ...
  useEffect(() => {
    if (fgRef.current && graphData.nodes.length > 0) {
      const timer = setTimeout(() => {
        const isMobile = window.innerWidth < 640;
        fgRef.current?.zoom?.(isMobile ? 1.2 : 1.5, 400);
      }, 500);
  ...
  if (loading) return <div className="text-center py-8 text-gray-500">Loading network graph...</div>;
  if (error || !data?.cooccurrence) return <div className="text-center py-8 text-red-600">Error: {error || 'No data available'}</div>;
  if (graphData.nodes.length === 0) return <div className="text-center py-8 text-gray-500">No co-occurrence data available</div>;

  return (
    <div className="w-full">
      <div className="mb-4 p-3 sm:p-4 bg-teal-50 border border-teal-200 rounded-lg">
        <h4 className="font-semibold text-teal-900 mb-2 text-sm sm:text-base">How to Read This Network:</h4>
      ...
      <div className="w-full border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
        <ForceGraph2D
          ...
          nodeLabel={(node: Record<string, unknown>) => {
            ...
            const connectedGenes = graphData.links.filter((l: Record<string, unknown>) => (l.source as string) === nodeId || (l.target as string) === nodeId).length;
